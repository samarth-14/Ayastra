...
import { getAnalyticsKPIs, getAnalyticsRevenueTrend, getAnalyticsCustomerGrowth, getAnalyticsCategoryRevenue, getAnalyticsHealth } from "../../../api";
...
export function DashboardAnalytics() {
  ...
  const [revenue, setRevenue] = useState(REV_12M);
  const [customers, setCustomers] = useState(CUSTOMERS);
  const [categories, setCategories] = useState(CATEGORY_REV);
  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      setLoading(true);
      try {
        const [kpiData, revenueTrendData, customerGrowthData, categoryRevenueData, healthData] = await Promise.all([
          getAnalyticsKPIs(),
          getAnalyticsRevenueTrend(),
          getAnalyticsCustomerGrowth(),
          getAnalyticsCategoryRevenue(),
          getAnalyticsHealth(),
        ]);

        if (kpiData && Array.isArray(kpiData)) {
          const formattedKpis = kpiData.map((k: any) => ({
            label: k.label || k.name || "KPI",
            value: k.value || "N/A",
            delta: k.delta || "+0%",
            color: k.color || "#3B82F6",
            icon: k.icon || <BarChart3 size={16} />,
          }));
          setKpis(formattedKpis);
        }

        if (revenueTrendData && Array.isArray(revenueTrendData)) {
          setRevenue(revenueTrendData);
        }

        if (customerGrowthData && Array.isArray(customerGrowthData)) {
          setCustomers(customerGrowthData);
        }

        if (categoryRevenueData && Array.isArray(categoryRevenueData)) {
          setCategories(categoryRevenueData);
        }
  ...
  return (
    <div style={{ padding: "1.5rem" }}>
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        <p style={{ color: "#8892a4", fontSize: "0.85rem", fontFamily: "'Inter',sans-serif" }}>FY 2025–26 · {loading ? "Loading..." : "Updated live"}</p>
      </div>

      {/* KPIs */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "0.75rem", marginBottom: "1.25rem", opacity: loading ? 0.6 : 1 }}>
        {kpis.map((k, i) => (
          <motion.div key={k.label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.07 }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ color: k.color, fontFamily: "'Space Grotesk',sans-serif", fontWeight: 800, fontSize: "1.35rem" }}>{loading ? "..." : k.value}</div>
       ...
       <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
         <div style={{ background: "rgba(11,17,32,0.8)", border: "1px solid rgba(59,130,246,0.12)", borderRadius: 16, padding: "1.25rem", opacity: loading ? 0.6 : 1 }}>
         ...
         <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
           <div style={{ background: "rgba(11,17,32,0.8)", border: "1px solid rgba(59,130,246,0.12)", borderRadius: 16, padding: "1.25rem", flex: 1, opacity: loading ? 0.6 : 1 }}>
           ...
           <div style={{ background: "rgba(11,17,32,0.8)", border: "1px solid rgba(59,130,246,0.12)", borderRadius: 16, padding: "1.25rem", flex: 1, opacity: loading ? 0.6 : 1 }}>
             ...
             <ResponsiveContainer width="100%" height={90}>
               <BarChart data={categories} layout="vertical" margin={{ top: 0, right: 5, bottom: 0, left: -5 }}>
       ...
       {/* Business Health */}
       <div style={{ background: "rgba(11,17,32,0.8)", border: "1px solid rgba(139,92,246,0.15)", borderRadius: 16, padding: "1.25rem", opacity: loading ? 0.6 : 1 }}>
