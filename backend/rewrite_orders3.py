...
const STATUS_FLOW: Record<string, string> = {
  pending: "confirmed", confirmed: "processing", processing: "dispatched", dispatched: "delivered",
};
const STATUS_NEXT_LABEL: Record<string, string> = {
  pending: "Confirm", confirmed: "Process", processing: "Dispatch", dispatched: "Deliver",
};
...
export function DashboardOrders() {
  ...
  const handleStatusUpdate = async (orderId: number, currentStatus: string) => {
    ...
    try {
      await updateOrderStatus(String(orderId), nextStatus);
      await fetchOrders();
    } catch (e) {
    ...
    <div style={{ padding: "1.5rem" }}>
      ...
      <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.4 }}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6} style={{ textAlign:"center", padding:"3rem", color:"#4B5563" }}>Loading orders...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign:"center", padding:"3rem", color:"#4B5563" }}>No orders yet. Create your first order!</td></tr>
            ) : filtered.map((o, i) => {
                    ...
                    {nextStatus ? (
                      <motion.button
                        ...
                        style={{
                          ...
                          background: nextColor ? nextColor + "18" : "rgba(59,130,246,0.1)",
                          border: "1px solid " + (nextColor ? nextColor + "40" : "rgba(59,130,246,0.3)"),
                          borderRadius: 6, padding: "4px 10px",
                          color: nextColor || "#3B82F6",
                          fontSize: "0.72rem", fontWeight: 700,
                          cursor: isUpdating ? "not-allowed" : "pointer",
                          opacity: isUpdating ? 0.6 : 1,
                        }}>
                        {isUpdating ? "..." : nextLabel}
                        {!isUpdating && <ChevronRight size={11} />}
                      </motion.button>
                    ) : (
                      <span style={{ color: "#22C55E", fontSize: "0.72rem", fontWeight: 700 }}>Complete</span>
                    )}
                  </td>
                </tr>
              );
