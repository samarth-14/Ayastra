...
import { getInventory, getInventoryKPIs, createInventoryItem } from "../../../api";
...
function AddSKUModal({ onClose, onSave }: { onClose: () => void; onSave: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        style={{ background: "#0b1120", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 16, padding: "1.5rem", width: "100%", maxWidth: 500 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
          <h3 style={{ color: "#e8eaf0", fontFamily: "Space Grotesk,sans-serif", fontWeight: 700 }}>Add New SKU</h3>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#8892a4", cursor: "pointer" }}>
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
            {[
              { key: "product_name", label: "Product Name", placeholder: "Brass Ganesh Statue" },
              { key: "sku", label: "SKU Code", placeholder: "BRS-GNS-001" },
              { key: "warehouse_name", label: "Warehouse", placeholder: "Main Warehouse" },
              { key: "unit", label: "Unit", placeholder: "MT" },
              { key: "quantity_on_hand", label: "Quantity", placeholder: "100", type: "number" },
              { key: "unit_cost", label: "Unit Cost (Rs)", placeholder: "500", type: "number" },
              { key: "reorder_point", label: "Reorder Point", placeholder: "20", type: "number" },
            ].map((f) => (
              <div key={f.key} style={{ gridColumn: f.key === "product_name" ? "1 / -1" : "auto" }}>
                <label style={{ display: "block", color: "#8892a4", fontSize: "0.75rem", marginBottom: 6, fontFamily: "Space Grotesk,sans-serif", fontWeight: 600 }}>
                  {f.label.toUpperCase()}
                </label>
                <input
                  type={f.type || "text"}
                  ...
                  required
                  style={{ width: "100%", background: "rgba(5,8,22,0.6)", border: "1px solid rgba(59,130,246,0.15)", borderRadius: 8, padding: "0.55rem 0.75rem", color: "#e8eaf0", fontSize: "0.85rem", outline: "none", boxSizing: "border-box" }}
                />
              </div>
            ))}

            <div>
              <label style={{ display: "block", color: "#8892a4", fontSize: "0.75rem", marginBottom: 6, fontFamily: "Space Grotesk,sans-serif", fontWeight: 600 }}>STATUS</label>
              <select value={form.status} onChange={(e) => setForm(p => ({ ...p, status: e.target.value }))}
                style={{ width: "100%", background: "rgba(5,8,22,0.6)", border: "1px solid rgba(59,130,246,0.15)", borderRadius: 8, padding: "0.55rem 0.75rem", color: "#e8eaf0", fontSize: "0.85rem", outline: "none" }}>
          ...
          <div style={{ display: "flex", gap: "0.75rem", justifyContent: "flex-end", marginTop: "0.5rem" }}>
            <button type="button" onClick={onClose}
              style={{ background: "transparent", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 8, padding: "0.6rem 1.25rem", color: "#8892a4", cursor: "pointer", fontFamily: "Space Grotesk,sans-serif", fontWeight: 600 }}>
              Cancel
            </button>
            <button type="submit" disabled={saving}
              style={{ background: "linear-gradient(135deg,#F59E0B,#D97706)", border: "none", borderRadius: 8, padding: "0.6rem 1.25rem", color: "#141C2E", cursor: saving ? "not-allowed" : "pointer", fontFamily: "Space Grotesk,sans-serif", fontWeight: 700 }}>
...
export function DashboardInventory() {
  ...
  return (
    <div style={{ padding: "1.5rem" }}>
      <AnimatePresence>
        {showModal && <AddSKUModal onClose={() => setShowModal(false)} onSave={fetchData} />}
      </AnimatePresence>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} style={{ textAlign: "center", padding: "3rem", color: "#4B5563" }}>No items found. Click Add SKU to get started.</td></tr>
            ) : filtered.map((item, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: "1px solid rgba(59,130,246,0.05)", cursor: "pointer" }}
                  ...
                  <td style={{ padding: "0.85rem 1rem", color: "#8892a4", fontSize: "0.78rem" }}>{item.warehouse_name || "-"}</td>
                  <td style={{ padding: "0.85rem 1rem", color: "#e8eaf0", fontWeight: 600, fontSize: "0.85rem" }}>{item.quantity_on_hand || 0} {item.unit || "MT"}</td>
                  <td style={{ padding: "0.85rem 1rem", color: "#8892a4", fontSize: "0.78rem" }}>Rs.{item.unit_cost || 0}</td>
                  <td style={{ padding: "0.85rem 1rem" }}>
